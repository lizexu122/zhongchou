...
public class ProjectDaoImpl implements ProjectDao {
    ...
    @Override
    public void updatePraise(Project project, int action) {
        Session session = sessionFactory.getCurrentSession();
        Query query =null;
        switch (action) {
            case 1:
                query = session.createQuery("update Project set praise = praise +1 where id =?");
                break;
            case -1:
                query = session.createQuery("update Project set praise = praise -1 where id =?");

        }
        query.setString(0,project.getId());
        query.executeUpdate();
    }
